...
if ($_SERVER['REQUEST_METHOD'] == 'GET')
{
  if (isset($_GET['id_cargamento'])) {
    $sql = $dbConn2->prepare("SELECT * FROM pesaje where id_cargamento=:id_cargamento");
    ...
    if ($sql->rowCount() == 0) {
      ...
      //Mostrar lista 
      $sql = $dbConn2->prepare("SELECT * FROM pesaje");
      $sql->execute();
...
{
    $input = $_POST;
    $sql = "INSERT INTO pesaje
          (id_cargamento, no_parcialidad, peso_parcialidad)
          VALUES
          (:id_cargamento, :no_parcialidad, :peso_parcialidad)";
